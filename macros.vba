...
Public Sub Color_AllNames_InitialDotSurname()
    ...
    Dim dlg As frmOptions
    Set dlg = New frmOptions
    dlg.Show vbModal
    ...
    Dim onlyThisSlide As Boolean
    colorLines = dlg.ColorLines
    onlyThisSlide = dlg.OnlyThisSlide
    Unload dlg

    Dim totalColored As Long, totalFixed As Long

    If onlyThisSlide Then
        Dim sld As Slide
    ...
    Else
        ...
        For Each s In ActivePresentation.Slides
            totalColored = totalColored + ProcessShapesCollection(s.Shapes, targetColor, totalFixed, colorLines)
        Next s
    End If
